...
| `POST /posts/upload` | PDF/엑셀 업로드(multipart: `file`, `content_type`, `category`, `subtype?`, `capture_images?`(같은 키로 여러 장 가능)). 저장 → `fileparser`로 텍스트 추출 → 성공 시 `status=researched`, 실패 시 `status=failed_file_parsing` + 에러 메시지. 둘 다 201 응답, 파일은 `UPLOAD_DIR`에 저장. PDF는 텍스트와 함께 내장 이미지도 추출(`fileparser.ExtractPDFImages`, 비치명적). `capture_images`는 PDF에 없는 정보(공고 사이트의 표·지도 등)를 사용자가 직접 캡처해 첨부하는 선택 필드로, 파일 종류(PDF/엑셀) 무관하게 받는다(`handler.saveCaptureImages`, 비치명적, 지원 확장자 아니면 조용히 스킵) → 둘 다 같은 목록으로 합쳐져 `research_results.raw_data`에 `{"images": [...]}`로 기록 |
...
PDF 추출 이미지와 사용자가 업로드한 웹페이지 캡처 이미지(`capture_images`)는 동일한 `images` 목록으로 합쳐져 위 필터링을 거쳐 LLM에 전달된다 — 별도 구분 없이 같은 vision 파이프라인을 탄다. 어떤 캡처가 "정보를 텍스트로 읽어야 할 표"이고 어떤 캡처가 "그대로 삽입해도 될 사진/지도"인지는 시스템 프롬프트로 LLM이 판단한다(`llm.draftSystemPrompt`).
