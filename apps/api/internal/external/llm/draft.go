...
- 가독성을 위해 큰 섹션이 바뀔 때마다 마크다운 구분선(---)을 넣고, 글 도입부나 핵심 요약에 해당하는 한두 문장은 인용구(> ...) 형식으로 한 번 강조합니다.
- 이미지는 새로 만들어내지 않습니다. 메시지에 원문 PDF에서 추출된 이미지나 사용자가 첨부한 웹페이지 캡처 이미지가 실제로 첨부되어 있다면, 그 이미지들을 직접 보고 판단합니다. 공급정보 표·일정표처럼 텍스트/표 정보가 담긴 페이지 캡처는 그 안의 내용을 읽어서 본문 텍스트나 마크다운 표로 정리하고, 그 캡처 이미지 자체를 본문에 삽입하지 않습니다. 위치를 보여주는 지도나 실사진처럼 그대로 보여줘도 되는 이미지만, 본문과 실제로 관련 있는 경우에 한해 삽입 대상으로 선택합니다. 장식용 아이콘·화살표·불릿 같은 이미지는 제외합니다. 삽입할 때는 각 이미지 직전에 표시된 "파일명: ..." 텍스트의 파일명을 그대로 써서 ![대체텍스트](파일명) 형식으로 본문에 넣습니다. 그 파일명이 아닌 다른 파일명을 지어내지 않고, 관련 있는 이미지가 없으면 삽입하지 않습니다.
